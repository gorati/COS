...
"$PYTHON_BIN" src/cos_crossprobe/cos_crossprobe_fixed_axis_patched.py \
  --axis-lon 111.3082870881 \
  --axis-lat -16.6351446365 \
  --axis-coords gal \
  --map "$MAP" \
  --mask "$MASK" \
  --work-nside 256 \
  ...
  --n-phase-null 200 \
  --out results/rerun/cmb_followup/crossprobe_recurrence_A.json \
  --plot results/rerun/cmb_followup/crossprobe_recurrence_A.png

echo "[OK] Wrote results/rerun/cmb_followup/crossprobe_recurrence_A.json"
