...
_Q0_NULL_WORKER_STATE: Dict[str, Any] = {}
...
def fit_cosmography_grid(
    ...
    if denom == 0.0 or not np.isfinite(denom):
        raise AnalysisError("Degenerate weighted-mean denominator encountered in q0 fit.")

    dl_prefactor = C_KM_S / DEFAULT_H0
    dl_grid = dl_prefactor * (z[None, :] + 0.5 * (1.0 - qgrid[:, None]) * z[None, :] * z[None, :])
    if np.any(dl_grid <= 0.0):
        raise AnalysisError("Non-positive luminosity distance encountered.")
    mu_grid = 5.0 * np.log10(dl_grid) + 25.0

    resid_noM = m_obs[None, :] - mu_grid
    rhs = resid_noM @ y
    M_vals = rhs / denom
    tmp = resid_noM @ inv_cov
    chi2_vals = np.einsum("ij,ij->i", tmp, resid_noM) - (rhs * rhs) / denom
...
def valid_axes_from_membership(pos_matrix: np.ndarray, min_hemi_size: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    n_pos_arr = np.sum(pos_matrix, axis=0, dtype=int)
    n_neg_arr = pos_matrix.shape[0] - n_pos_arr
    valid = (n_pos_arr >= min_hemi_size) & (n_neg_arr >= min_hemi_size)
    return valid.astype(bool), n_pos_arr.astype(int), n_neg_arr.astype(int)
...
def absolute_axis_stat_values(
    ...
    if stat_name == "abs_delta_mean_residual":
        return np.abs(residual_stat_from_coeff(coeff, residuals))
    if stat_name == "abs_delta_q0":
        return np.abs(
            compute_delta_q0_for_axes(
                z=z,
                m_obs=m_obs,
                cov=cov,
    ...
    raise AnalysisError(f"Unsupported statistic for null calibration: {stat_name}")
...
def _init_q0_null_worker(
    z: np.ndarray,
    m_obs: np.ndarray,
    cov: np.ndarray,
    base_pos: np.ndarray,
    min_hemi_size: int,
    progress_every_axes: int,
) -> None:
    global _Q0_NULL_WORKER_STATE
    _Q0_NULL_WORKER_STATE = {
        "z": z,
        "m_obs": m_obs,
        "cov": cov,
        "base_pos": base_pos,
        "min_hemi_size": int(min_hemi_size),
        "progress_every_axes": int(progress_every_axes),
    }


def _compute_q0_null_from_seed(seed: int) -> Tuple[float, float]:
    state = _Q0_NULL_WORKER_STATE
    rng = np.random.default_rng(int(seed))
    n_obj = state["base_pos"].shape[0]
    perm = rng.permutation(n_obj)
    pos_perm = state["base_pos"][perm, :]
    valid_i, _, _ = valid_axes_from_membership(pos_perm, state["min_hemi_size"])
    vals_abs = np.abs(
        compute_delta_q0_for_axes(
            z=state["z"],
            m_obs=state["m_obs"],
            ...
            pos_matrix=pos_perm,
            valid_axes=valid_i,
            progress_every=state["progress_every_axes"],
        )
    )
    return float(vals_abs[0]), float(np.nanmax(vals_abs[valid_i]))
...
def sky_scramble_null(
    ...
    progress_every_axes = max(progress_every * 10, 1000)

    if stat_name == "abs_delta_q0":
        valid0, _, _ = valid_axes_from_membership(base_pos, min_hemi_size)
        data_abs = np.abs(
            compute_delta_q0_for_axes(
                z=z,
                m_obs=m_obs,
                ...
                pos_matrix=base_pos,
                valid_axes=valid0,
                progress_every=progress_every_axes,
            )
        )
        warn(
            "Sky-scramble null for abs_delta_q0 is exact but very slow with the full covariance matrix. "
            "Using the exact-fast path that skips unused residual-weight operators."
        )
    else:
        coeff0, valid0, *_ = prepare_residual_mean_operator(
            cov,
            base_pos,
            min_hemi_size,
            progress_every=progress_every_axes,
        )
        data_abs = absolute_axis_stat_values(
            stat_name=stat_name,
            z=z,
            m_obs=m_obs,
            cov=cov,
    ...
    if stat_name == "abs_delta_q0" and int(n_jobs) > 1:
        seed_seq = np.random.SeedSequence(int(null_seed))
        child_seeds = [int(ss.generate_state(1, dtype=np.uint64)[0]) for ss in seed_seq.spawn(n_null)]
        n_jobs_eff = max(1, int(n_jobs))
        log(f"Sky-scramble q0 null running with n_jobs={n_jobs_eff}")
        with mp.Pool(
            processes=n_jobs_eff,
            initializer=_init_q0_null_worker,
            initargs=(z, m_obs, cov, base_pos, min_hemi_size, progress_every_axes),
        ) as pool:
            for i, (fixed_val, global_val) in enumerate(
                pool.imap(_compute_q0_null_from_seed, child_seeds, chunksize=1)
            ):
                fixed_cos_null.append(float(fixed_val))
                global_max_null.append(float(global_val))
                if (i + 1) % progress_every == 0 or i == 0 or (i + 1) == n_null:
        ...
        for i in range(n_null):
            ...
            if stat_name == "abs_delta_q0":
                valid_i, _, _ = valid_axes_from_membership(pos_perm, min_hemi_size)
                vals_abs = np.abs(
                    compute_delta_q0_for_axes(
                        z=z,
                        m_obs=m_obs,
                ...
                coeff_i, valid_i, *_ = prepare_residual_mean_operator(
                    cov, pos_perm, min_hemi_size, progress_every=progress_every_axes
                )
                vals_abs = absolute_axis_stat_values(
                    stat_name=stat_name,
                    z=z,
                    m_obs=m_obs,
                    cov=cov,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--n-jobs", type=int, default=1, help="Number of worker processes for the exact sky-scramble q0 null")
